...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($name === '' || strlen($name) > 160 || strlen($email) > 255 || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $error = 'Please provide a valid name and email.';
    } elseif (strlen($notes) > 2000) {
        $error = 'Delivery notes must be 2,000 characters or fewer.';
    } elseif (submitBookOrder($name, $email, $notes, $items)) {
        clearCart();
        unset($_SESSION['_csrf']);
        $sent = true;
    } else {
        $error = 'The reading-room service is temporarily unavailable or one of the selected titles is no longer available. Please review the cart and try again.';
        $items = cartItems();
    }
}
?>
<main class="hq-section intake-page store-page">
  ...
  <?php if ($error): ?><p class="intake-error" role="alert"><?= e($error) ?></p><?php endif; ?>
  <div class="checkout-grid"><div class="intake-panel"><form method="post" class="contact-form intake-form"><input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>"><label>Your name<input name="name" maxlength="160" required value="<?= e($_POST['name'] ?? '') ?>"></label><label>Email address<input type="email" name="email" maxlength="255" autocomplete="email" required value="<?= e($_POST['email'] ?? '') ?>"></label><label>Delivery notes<textarea name="notes" rows="5" maxlength="2000" placeholder="City, preferred format, or anything useful…"><?= e($_POST['notes'] ?? '') ?></textarea></label><button class="hq-button hq-button--primary" type="submit">Send book inquiry ↗</button></form></div><aside class="checkout-aside"><span class="mono-label">YOUR SHELF</span><?php foreach ($items as $item): ?><p><?= e($item['title']) ?> × <?= (int)$item['quantity'] ?></p><?php endforeach; ?><div class="dossier-rule"></div><strong><?= e(number_format(cartTotal(), 2)) ?> <?= e($items[0]['currency'] ?? 'USD') ?></strong></aside></div>
  <?php endif; ?>
</main>
